...
async def get_shards_info(collection_name: str) -> List[dict]:
    """
    Get document count, replica count per shard for a given collection.

    For each shard, counts total documents and number of available replica members.
    Uses sh.status() to get replica configuration info without direct connections.

    Args:
        collection_name: Name of the collection

    Returns:
        List of dicts with shardName, docs, replicas info
    """
    collection = db.get_collection(collection_name)
    cursor = collection.find()
    explain_result = await cursor.explain()

    execution_stages = explain_result.get("executionStats", {}).get("executionStages", {})
    shard_docs = []

    shards_list = await client.admin.command("listShards")
    
    # Get shard info from sh.status() to count replicas
    shards_config = {}
    for shard in shards_list.get("shards", {}):
        shard_name = shard["_id"]
        host = shard["host"]
        # host format: "shard1/shard1-1:27018,shard1-2:27018,shard1-3:27018"
        # Count members after the replica set name
        members = host.split("/")[1].split(",")
        shards_config[shard_name] = {"replicas": len(members), "members": members}

    for shard_stat in execution_stages.get("shards", []):
        shard_name = shard_stat["shardName"]
        shard_config = shards_config.get(shard_name, {})
        
        shard_docs.append({
            "shard": shard_name,
            "docs": shard_stat.get("nReturned", 0),
            "replicas": shard_config.get("replicas", 1)
        })
...
async def root():
    ...
    if topology_type == "Sharded":
        ...
        # Get docs and replicas per shard for all collections
        shard_docs = {}
        for collection_name in collections.keys():
            shard_info = await get_shards_info(collection_name)
            for s in shard_info:
                if s["shard"] not in shard_docs:
                    shard_docs[s["shard"]] = {
                        "docs": s["docs"],
                        "replicas": s["replicas"]
                    }
                else:
                    shard_docs[s["shard"]]["docs"] += s["docs"]
...
async def collection_count(collection_name: str):
    ...
    shards_docs = await get_shards_info(collection_name)
    return {
        "status": "OK",
        "mongo_db": DATABASE_NAME,
        "items_count": items_count,
        "shards_docs": shards_docs
    }
