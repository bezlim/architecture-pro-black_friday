...
async def root():
    ...
    if topology_type == "Sharded":
        ...
        # Get docs per shard via explain() for all collections
        shard_docs = {}
        for collection_name in collections.keys():
            ...
            for shard_stat in execution_stages.get("shards", []):
                shard_docs.setdefault(shard_stat["shardName"], {})[collection_name] = shard_stat.get("nReturned", 0)
...
async def collection_count(collection_name: str):
    ...
    shards_docs = await get_shards_docs(collection_name)
    return {
        "status": "OK",
        "mongo_db": DATABASE_NAME,
...
async def get_shards_docs(collection_name: str) -> List[dict]:
    """
    Get document count per shard for a given collection using explain().
    
    Args:
        collection_name: Name of the collection
        
    Returns:
        List of dicts with shardName and nReturned for each shard
    """
    collection = db.get_collection(collection_name)
    cursor = collection.find()
    explain_result = await cursor.explain()
    ...
    return [
        {
            ...
            "docs": shard.get("nReturned", 0)
        }
        for shard in execution_stages.get("shards", [])
    ]
